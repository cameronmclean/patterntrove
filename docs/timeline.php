...
$sql = "SELECT ".db_timestamp("min(bit_datestamp)")." AS start,  ".db_timestamp("max(bit_datestamp)")." AS end FROM blog_bits ".$sql_where;

$tresult = runQuery($sql,'Fetch Page Groups');
$posts = db_get_next_row($tresult);

$tlcolors = array("red","blue","green","gray","deeppink","orange","purple","cyan","goldenrod","light-gray","black");

if(!isset($head)) { $head = ''; }
if(!isset($bodytag)) { $bodytag = ''; }
