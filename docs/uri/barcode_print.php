...
$sql = "SELECT * FROM " . db_uri_db("uri") . " WHERE  uri_id = {$id} " . db_limit_1();
					$tresulta = runQuery($sql,'Fetch Page Groups');
 					$line_uri = db_get_next_row($tresulta);

$uri = $line_uri['uri_url'];

$sql = "SELECT *, ".db_timestamp("bit_timestamp")." AS datetime FROM  blog_bits WHERE  bit_uri ={$id} AND  bit_edit = 0";
		$tresulta = runQuery($sql,'Fetch Page Groups');
...
$sql = "SELECT * FROM " . db_uri_db("printers") . " WHERE  print_id = ".$_REQUEST['printer'];
$tresulta = runQuery($sql,'Fetch Page Groups');
 					$line = db_get_next_row($tresulta);
if($line['print_id']){
