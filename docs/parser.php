...
//	$row = db_get_next_row($tresult);
	$row = db_get_post_by_id($postid, 'edit');
	...
	$new_id = edit_blog_new($postid, 'save', 'Sketch',NULL, NULL, $metad, NULL);
...
if ( $_REQUEST['image'] &&  $_REQUEST['originalID']) {
	...
	$sql = "SELECT * FROM blog_data WHERE data_id = $originalID";
	...
	$sql = "UPDATE blog_data SET data_data = '".addSlashes($content)."' WHERE data_id = '$originalID'";
