...
if(strlen($uri[0])){
	$sql = "SELECT * FROM  blog_blogs WHERE blog_sname = '".addslashes($uri[0])."'";
...
if(in_array($collection, $supports)){
	switch($collection){
		case "Posts": 
			...
			$where .= " AND bit_blog = '{$blog['blog_id']}'";
			...
			$sql = "SELECT  bit_id  ,  bit_user ,  bit_title ,  bit_content , ".db_timestamp("bit_datestamp")." AS datetime, ".db_timestamp("bit_timestamp")." AS timestamp, blog_blogs.blog_zone, blog_blogs.blog_id, bit_meta, bit_group, bit_cache
				$sqlb";
...
if($countblog > $atompagelimit){
	if($request['page']){
		$limitt = $atompagelimit*($request['page']-1)." OFFSET ".$atompagelimit;
		}else{
  ...
  <edmx:DataServices xmlns:m="http://schemas.microsoft.com/ado/2007/08/dataservices/metadata" m:DataServiceVersion="2.0">
    <Schema Namespace="LabTroveOData" xmlns:d="http://schemas.microsoft.com/ado/2007/08/dataservices" xmlns:m="http://schemas.microsoft.com/ado/2007/08/dataservices/metadata" xmlns="http://schemas.microsoft.com/ado/2007/05/edm">
      <EntityType Name="Post">
		...
		<Property Name="Author" Type="LabTroveOData.Author" Nullable="false"/>
	</EntityType>
