...
function run() {
    ...
    if ($response->status == Auth_OpenID_CANCEL) {
		...
		$sql = "SELECT * FROM users WHERE user_openid LIKE '".addslashes($user['user_openid'])."' ".db_limit_1();
		$tresult = runQuery($sql,'iGet User Info');
		if(db_get_number_of_rows ( $tresult ) ){
			$user_sql = db_get_next_row($tresult, MYSQL_ASSOC);
			$user = array_merge($user,$user_sql);
			$sql = "UPDATE users SET user_fname =  '".addslashes($user['user_fname'])."', user_email =  '".addslashes($user['user_email'])."', user_image = '".addslashes($user['user_image'])."' WHERE  users.user_id ={$user['user_id']} " . db_limit_1();
...
if($_REQUEST['submitinfo']){
	...
	if($_SESSION['regist_user']['handle']==$_REQUEST['key']){
		...
		if($fail){
			...
			$sql = "UPDATE users SET user_fname =  '".addslashes($_SESSION['regist_user']['user_fname'])."', user_email =  '".addslashes($_SESSION['regist_user']['user_email'])."', user_enabled = 1 WHERE users.user_name = '{$_SESSION['regist_user']['user_name']}' " . db_limit_1();
		runQuery($sql,'update user info');
			...
			if($_SESSION['turl']){
			 ...
			 header("Location: {$ct_config['blog_path']}"); exit();
			}
		}
