...
$pathinfo = substr($_SERVER['LABTROVE_REQUEST_PATH'],6);
...
if($_SESSION['user_name'] && $request['subscription'] && ($request['subscription'] == $_SESSION['user_name'])){
	...
	$where = " INNER JOIN  blog_sub ON  blog_bits.bit_blog =  blog_sub.sub_blog 
...
<?php
$sql = "SELECT  bit_id as uid ,bit_id  ,  bit_user ,  bit_title ,  bit_content , ".db_timestamp("bit_datestamp")." AS datetime, blog_blogs.blog_zone, blog_blogs.blog_id
FROM  blog_bits 
...
$sql = "SELECT  blog_com.com_id as uid,  blog_bits.bit_id ,  blog_com.com_user AS  bit_user ,  blog_com.com_title AS  bit_title ,  blog_com.com_cont AS  bit_content , ".db_timestamp( "blog_com.com_datetime" )." AS datetime , 'comment' AS btype , blog_com.com_edit, blog_blogs.blog_zone, blog_blogs.blog_id
FROM  blog_bits 
