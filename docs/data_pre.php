...
$sql = "SELECT   data_id ,  data_post ,  data_datetime ,  data_type  
		FROM blog_data WHERE data_id = '$id'";
		
$result = runQuery($sql,'Fetch Page Groups');
$datainfo = db_get_next_row($result);

if($datainfo['data_post']){
	$sql = "SELECT bit_blog FROM blog_bits WHERE bit_id = '{$datainfo['data_post']}' AND bit_edit = 0";
	$result = runQuery($sql,'Fetch Page Groups');
	$postid = db_get_next_row($result);
...
if($_SESSION['blog_id']){
	$sql = "SELECT *,'data_pre' FROM  blog_blogs WHERE blog_id = '{$_SESSION['blog_id']}'";
...
if(1){
	...
	$sql = "SELECT data_data, data_post, data_type, data_id, data_datetime FROM blog_data WHERE data_id = $id";
	$result = runQuery($sql,'Fetch Page Groups');
	...
	switch ($row['data_type']) {
		...
		case 'jpeg':
			...
			echo $row['data_data'];
			$sql = "SELECT data_data, data_type FROM blog_data WHERE data_id = '$id'";break;
...
END;
		...
		$img = ' <div id="source_container" align="center" style="padding-top:'.$imgheight.'px; padding-bottom:'.$imgheightb.'px;"><img src="'.$imagepath.'" width="'.$sizex.'" height="'.$sizey.'" alt="source image" /></div>
		        ...
		        <input type="button" style="float:right;" value="Post Comment" onclick="postComment('.$id.',window.location.href)"/>	
		        <input type="button" style="float:right;" value="Cancel Comment" onclick="canComment(window.location.href)"/>';
		...
		$img = "<div align=\"center\" style=\"padding-top:".$imgheight."px;padding-bottom:".$imgheight."px;\"><img  alt=\"\" src=\"".$imagepath."\" width=\"$sizex\" height=\"$sizey\"/></div>\n\n";
		}	
	}
	...
	if($metadata['METADATA']['OVERLAY']) {
		...
		$overlay_links = "\t\t<span>This Image has comments (graphical overlays)</span>\n\n";
		$datas = split(",",$metadata['METADATA']['OVERLAY']);
		$table_cols = 3;
		$col = 0;

		$overlay_links .= "<table width=\"640\">";
		foreach($datas as $bit){
			$col++;
			...
			//Get user's full name, as a userid is long and meaningless
			$overlay_user = db_get_user($overlay['METADATA']['USER']);			
			$overlay_links .= "\t\t<tr><td><input type=\"checkbox\" name=\"overlay\" onclick=\"toggleDisplay('id_".$overlay['METADATA']['ID']."')\"/> ".$overlay['METADATA']['TITLE']." - ".$overlay_user['user_fname']." </td></tr>\n\n";
		}
		$overlay_links .= "</table>";
	...
	if($metadata['METADATA']['OVERLAY']) {
		...
		foreach ($overlayIDs as $overlayID) {
			...
			$body.= "\t\t<div id=\"id_".$overlayID."\" style=\" display:none; position: relative; z-index:".(10+($f++))."; height:0px;\">
						<img alt=\"\" src=\"{$ct_config['blog_path']}data/files/{$overlayID}.png\"/></div>\n\n";
...
$body .= "\t<br/><div class=\"info_image\" style=\"width:640px;\">\n\n";
	...
	if ($_SESSION['user_name'] && $distype != "timeplot" && !$not_viewable) {
		...
		$body .="\t\t<a href=\"{$ct_config['blog_path']}data/$id.html?comment=1\">Add comment</a><br/><br/>";
	...
	if(isset($datalist)){
		...
		foreach( $datalist as $key => $value){
			...
			$body .= "\"><img alt=\"\" src=\"".file_img(substr($key,5))."\" style=\"vertical-align:middle;\"/> ".substr($key,5)."</a>&nbsp; ";
