...
if(isset($_SESSION['user_name'])){
	...
	if(!isset($_REQUEST['flogin']) || !$_REQUEST['flogin']){
        ...
        $ret .= '<div id="loginBox" style="display: '.$display.'"><form action="'.$ct_config['blog_path'].'login.php" method="post">'.
		'<input type="hidden" name="rew" value="'.$ct_config['var_prefix'].'">'.
		'<input type="hidden" name="turl" value="'.$turl.'">'.
		'Username <input type="text" size="7" name="usern" class="main_input" tabindex="1" align="absmiddle">'.
		'Password <input type="password" name="pass" size="10" tabindex="2" align="absmiddle" class="main_input">'.
		'Remember Me <input name="remember" type="checkbox" value="true">'.
		'<button type="submit" name="loginsub" value=">" class="main_submit">&gt;</button></form></div>';
...
function user_info_display(){
	...
	if($_REQUEST['pass_udate']){
		$user = db_get_user($_SESSION['user_name'], 1);
		if($user['user_pass'] != crypt(stripslashes($_REQUEST['pass_curr']),$user['user_pass'])){
			...
			$sql = "UPDATE  users SET  user_pass =  '".addslashes(crypt(stripslashes($_REQUEST['pass_new'])))."'  WHERE  user_name LIKE  '{$_SESSION['user_name']}' " . db_limit_1();
...
function getUsers(){
	...
	if($_REQUEST['add_user']){
			...
			$user = db_get_user($_REQUEST['user_name'], 1);
			if($user['user_id']){
	...
	if($_REQUEST['user_edit'] || $_REQUEST['user_udate']){
		if($_REQUEST['user_udate']){
			if($_REQUEST['user_fname'] && $_REQUEST['user_fname']){
				$sql = "UPDATE  users SET  user_fname =  '{$_REQUEST['user_fname']}', user_email =  '{$_REQUEST['user_email']}', user_type =  '{$_REQUEST['user_type']}' WHERE  users.user_id = '{$_REQUEST['user_edit']}';";
				runQuery($sql,'iGet User Info');
				$ok = 1;
				if($_REQUEST['pass_new']){
					...
					if(strlen($_REQUEST['pass_new'])<6){
						...
						$sql = "UPDATE  users SET  user_pass =  '".addslashes(crypt(stripslashes($_REQUEST['pass_new'])))."'  WHERE  users.user_id = '{$_REQUEST['user_edit']}' " . db_limit_1();
						 runQuery($sql,'iGet User Info');
						$ok = 1;
					}
					if(!$ok){
		...
		if(!$ok){
		$user = db_get_user_by_id($_REQUEST['user_edit']);
		...
		$blogpost['post'] .= "</form>";
		//$ret .= blog_style_post($blogpost); // done at the end too!
		}
	}
	...
	$tresult = db_get_users();
	while($user = db_get_next_row($tresult)){
