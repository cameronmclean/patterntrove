...
function check_remembered(){
	global $ct_config;
	if(isset($_COOKIE['user_name']) && $usern = $_COOKIE['user_name']){
...
$ret .= '<div id="openid" style="">
	...
	<!-- Simple OpenID Selector -->
	<form action="'.$ct_config['blog_path'].'openid/try_auth.php?zone='.$_REQUEST['zone'].'" method="get" id="openid_form">
...
function user_info_display_by_user_name($user_name){
	...
	if(isset($_REQUEST['update_userdetails']) && $_REQUEST['update_userdetails']==$user_name) {

		if(($_SESSION['user_name'] == $user_name || $_SESSION['user_admin'] >2)){
			if(!isset($_REQUEST['user_fname']) || !strlen($_REQUEST['user_fname'])){
				newMsg("Please enter a Full Name","warning");
				$_REQUEST['edit']=1;
			}
			elseif(!isset($_REQUEST['user_email']) || !validate_email($_REQUEST['user_email'])){
				newMsg("Please enter a valid email","warning");			$_REQUEST['edit']=1;
			}else{
				
				if( $_SESSION['user_admin'] >2){
					 $user_admin = ", user_type = ".(int)$_REQUEST['user_type'];
					if($user_name == $_SESSION['user_name'])
		             $_SESSION['user_admin'] = $_REQUEST['user_type'];
				}else{
					$user_admin = "";
				}


				$sql = "UPDATE users SET user_fname =  '".addslashes($_REQUEST['user_fname'])."', user_email =  '".addslashes($_REQUEST['user_email'])."' $user_admin  WHERE users.user_name = '{$user_name}' " . db_limit_1();
			runQuery($sql,'update user info');
			if($user_name == $_SESSION['user_name']){
				 $_SESSION['user_fname'] = $_REQUEST['user_fname'];
	...
	if(isset($_REQUEST['edit']) && ($_SESSION['user_name'] == $user_name || $_SESSION['user_admin'] >2)){
		$edit = true;
		$blogpost['post'] .= "<form action=\"user/{$user_name}\" method=\"POST\"><input type=hidden name=update_userdetails value=\"$user_name\"/>";
	}else{
		$edit =false;
	}
	...
	if($edit && $_SESSION['user_admin'] >2){
		$blogpost['post'] .= "<tr><th>User Type:</th><td><select name=\"user_type\">";
	...
	if($edit){
		$blogpost['post'] .= "<tr><th></th><td align=right>(<a href=\"user/{$user_name}\">cancel</a>) <input type=submit name=save value=\"Save\"/></td></tr>";	
	}elseif($_SESSION['user_name'] == $user_name || $_SESSION['user_admin'] >2){
		$blogpost['post'] .= "<tr><th></th><td align=right>(<a href=\"user/{$user_name}?edit=1\">edit details</a>)</td></tr>";		
	}
...
function getUsers(){
	...
	while($row = db_get_next_row($result)){
		$user_type = "user";
		if($row['user_type'] == 2) $user_type = "moderator"; // not sure if '2' is used
