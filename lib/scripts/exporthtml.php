...
// local relative version of blog_url
$ct_config['blog_url_local'] = '';

if ($argv[1] == 'bom')
{
  // eg 5-39.34.31:1-38.37.14.13.12.1:4-26)
  // : separated list of entries, with each entry being blogid-post_id.postid.postid
  $bom = $argv[2];
  $depth = (int) $argv[3];
  $hashkey = $argv[4];
}
...
{
  $blog_id = (int) $argv[1];
  $indexpost = $argv[2]; // will be a comma separated list of integers
  $depth = (int) $argv[3];
  $hashkey = $argv[4];
  $bom = 0;
}
...
if (
        (!$bom || !$depth || !$hashkey)
        &&
        (!$blog_id || !$indexpost || !$depth || !$hashkey)
)
{
  //echo "blog_id=$blog_id, indexpost=$indexpost, depth=$depth, hashkey=$hashkey, Error";
  exit(1);
}
//exit(1);
...
if ($bom)
{
  foreach (explode(':', $bom) as $target)
  {
    $t = explode('-', $target);
    $blog_id = $t[0];
    $posts = implode(',', explode('.', $t[1])); // needs to be , separated not . separated
    //echo "<br>[$target]->[$blog_id][$posts]<br>";
    $b = do_it($blog_id, $posts, $bom);
    $blog_names[ $b['blog_sname' ] ] = $b['blog_name'];
  }
  write_index_page("$datapath/index.html", $blog_names);
  $blog_names['index.html'] = 'index.html';
  $export_name = 'export';
}
else
{
  $b = do_it($blog_id, $indexpost, $bom);
  $blog_names[ $b['blog_sname' ] ] = $b['blog_name'];
  $export_name = $b['blog_sname' ];
}

$blog_names_str = implode(' ', array_keys($blog_names));

file_put_contents("$datapath/status", "compressing");
`cd $datapath; zip -q -r {$export_name}.zip {$blog_names_str}`;
$resp = "Complete (Download export*: <a href=\"{$ct_config['blog_url']}cache/blogdumps/{$hashkey}/{$export_name}.zip\">{$export_name}.zip</a>)";
...
`cd $datapath; rm -rf {$blog_names_str}`;

//`rm -rf $datapath`;
//`rm -rf $datapath/{$request['blog_sname']}`;

function do_it($blog_id, $indexpost, $bom) // export the blog and its posts, return the blog name
{
  global $ct_config, $datapath, $datapath_export, $resp;
  ...
  if (file_exists("../../config/blog_{$blog_id}.php"))
    include_once("../../config/blog_{$blog_id}.php");
  include_once("../../docs/style/{$ct_config['blog_style']}/blogstyle.php");
 ...
 $postsscaned = array();
  ...
  for ($i = 0; $i < $depth; $i++)
  {
    ...
    foreach ($posts2 as $postid)
    {
      ...
      {
        $sql = "SELECT  * FROM  blog_bits WHERE  bit_id = {$postid} AND  bit_edit = 0";
        $tresult = runQuery($sql, 'Fetch Page Count');
        if ($post = db_get_next_row($tresult))
        {
          $subposts = array();
          ...
          foreach ($subposts as $id)
          {
            ...
            {
              $sql = "SELECT  bit_id FROM  blog_bits WHERE  bit_id = {$postid} AND  bit_edit = 0 AND bit_blog = $blog_id";
  ...
  foreach ($posts as $postid)
  {
    ...
    if ($postid == -1)
    {
      ...
      $sql = "SELECT bit_id, " . db_timestamp("bit_datestamp") . " AS datetime
		FROM  blog_bits 
		WHERE  bit_blog = {$blog_id}
		AND  bit_edit =0
      ...
      $result = runQuery($sql, 'Blogs');
      $posts[] = -1;
      while ($xpost = db_get_next_row($result))
      {
    ...
    {
	  $sql = "SELECT  bit_id ,  bit_user ,  bit_title ,  bit_content ,  bit_meta ,  bit_datestamp ,  bit_timestamp ,  bit_group ,  bit_blog ,  bit_edit , bit_edituser, bit_editwhy , " . db_timestamp("bit_datestamp") . " AS datetime,  " . db_timestamp("bit_timestamp") . " AS timestamp, bit_md5 , bit_cache, bit_rid, bit_edituser FROM  blog_bits WHERE  bit_id = {$postid} AND  bit_edit = 0";
      $tresult = runQuery($sql, 'Fetch Page Count');
      if (($post = db_get_next_row($tresult)) === false)
      {
        continue;
      }
    }
   ...
   $fname = $post['bit_id'] . "-" .$name . ".html";
    ...
    {
		$blogpost['url'] = $ct_config['blog_url'];
    }
    ...
    if (in_array('claddier', $ct_config['plugins'])) {
      $blogpost['post'] .=  "<div class=\"postTools\">" . claddier_list_citations(claddier_linked_from($post['bit_id']), $linkeddiv, $post['bit_id']) . "</div>\n";
    }
    else {
    ...
    if ($metadata['METADATA']['DATA'])
    {
      ...
      foreach ($datas as $bit)
      {
        ...
        if ($test)
        {
          $blogpost['data'] .= "<span class=comment>";
        }
    ...
    if ($post['bit_id'] != -1)
    {
      $uri = "{$ct_config['blog_url_local']}uri/" . dechex(getbituri($post['bit_id']));
...
//		http://blog_dev.sidious.chem.soton.ac.uk/neutral_drift/534/Betaglu_rev.html
    ...
    $body = preg_replace('/\/data\/([0-9]+)\.html/i', "{$ct_config['blog_url_local']}data/\\1.html", $body);
    $body = preg_replace('/url\(\/getdata\.php([^\)]+)\)/i', "url({$ct_config['blog_url']}getdata.php\\1)", $body);
    $safe_blog_url = preg_replace('/\//', '\/', $ct_config['blog_url']);    
	$body = preg_replace("/{$safe_blog_url}/", "", $body);
	$safe_fname_url = $blog['blog_sname'] . '\/' . $post['bit_id'] . '\/' . $name . '.html';
	$body = preg_replace("/{$safe_fname_url}/", $fname, $body);	
    $body = preg_replace("/\"{$blog['blog_sname']}\/([0-9]+)\/([\w]+)\.html\"/", "\\1-\\2.html", $body);
    ...
    if ($postid != -1)
    {
      pageify(&$body, "{$datapath_export}/{$fname}", $blog['blog_name'], $blog['blog_desc']);
    }

    if ($postid == $indexpost && !$bom)
    {
      pageify(&$body, "{$datapath_export}/{$title_url}", $blog['blog_name'], $blog['blog_desc']);
    }
  }

  if($bom)
  {
    write_blog_index_page($datapath_export, $blog['blog_name']);
  }
...
function write_index_page($filename, $blog_names)
{
  $index_page = <<<INDEX
<html>
  <head>
...
INDEX;
  ...
  foreach ($blog_names as $sname => $name)
  {
    $index_page .= "<li><a href='$sname/index.html'>$name</a></li>\n";
  }
...
function write_blog_index_page($dirname, $blog_name)
{
  $index_page = <<<INDEX
<html>
  <head>
    <title>Export - $blog_name</title>
  </head>
  <body>
   <h1>$blog_name</h1>
   <ul>
INDEX;
  ...
  foreach ( glob("{$dirname}/*.html") as $filepath )
  {
    $filename = basename($filepath);
    $index_page .= "<li><a href='$filename'>$filename</a></li>\n";
  }
...
INDEX;
  ...
  file_put_contents("$dirname/index.html", $index_page);
}
...
function proclink($link, $url)
{
  ...
  {
    $sql = "SELECT blog_bits.*  FROM blog_bitsWHERE bit_id = $link AND bit_edit = 0";
    $result = runQuery($sql, 'Blogs');
...
function pageify($body, $path, $blog_title, $blog_desc)
{
  global $ct_config;
  if ($ct_config['blog_style'] != "default")
...
<div id="page">
	...
	<div id="header">
		<div id="sitetitle">{$ct_config['blog_title']}</div>
		<div id="blogtitle">
			<h1><a href="index.html" id="white">{$blog_title}</a></h1>
			<span class="description">{$blog_desc}</span></div>
	</div>
		<div id="content">
		{$body}
		<div class="clear"></div>
...
function exgetdata($bit, $text=0, $size=null)
{
  ...
  {
    if ($text == 1)
      $metadata['METADATA']['TITLE'] = "Data: " . $metadata['METADATA']['TITLE'];
...
function makedatapage($id)
{
  ...
  $sql = "SELECT data_data, data_type, data_id, data_datetime FROM blog_data WHERE data_id = $id";
  $result = runQuery($sql, 'Fetch Page Groups');
  $row = db_get_next_row($result);
  ...
  switch ($row['data_type'])
  {
    ...
    case 'jpg':
      ...
      echo $row['data_data'];
      $sql = "SELECT data_data, data_type FROM blog_data WHERE data_id = $id";
      break;
    ...
    case 'data_meta':
      ...
      if ($ext && $ext != "html")
      {
        ...
        {
          include_once("export/data/{$ext}.php");
        }
      }
...
//image
  ...
  if (isset($datalist))
  {
    ...
    foreach ($datalist as $key => $value)
    {
      ...
      {
        $body .= "\t\t<a href=\"" . $value['URL'];
      }

      $icon = file_img(substr($key, 5), true);
