...
$id = (int)$argv[3];
	...
	$sql = "SELECT * FROM  blog_data WHERE  data_id = {$id}";
	$tresult = runQuery($sql,'');
	$data = db_get_next_row($tresult);
	...
	$sql = "UPDATE  blog_data SET  data_data =  '".addslashes(writexml($mdata))."' WHERE  blog_data.data_id = {$id} " . db_limit_1();
	runQuery($sql,'');
