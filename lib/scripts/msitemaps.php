...
$sql = "SELECT * FROM  blog_blogs WHERE blog_redirect = ''";
$tresult = runQuery($sql,'Fetch Page Groups');
while($blog = db_get_next_row($tresult)){
	if(checkzone($blog['blog_zone'],1)){
		...
		$sql = "SELECT *, ".db_timestamp("bit_timestamp")." as unix_time FROM  blog_bits WHERE  bit_blog = {$blog['blog_id']} AND  bit_edit =0 
