...
{
	global $ct_config;
	if($ct_config['upload_php'] == 'upload_php')
	{
		move_uploaded_file($from, $to);
	}
	else // cant be sure how the file made it here
	{
		rename($from, $to);
	}
}
...
function data_item_delete($post_id, $data_id){
	$row = db_get_post_by_id($post_id, 'edit');
	$metadata = readxml($row['bit_meta']);

	$newdata = array();
	$dataitems = explode(",",$metadata['METADATA']['DATA']);
	foreach($dataitems as $item){
		if($item == $data_id) continue;
		$newdata[] = $item;
	}
	

	
	$metadata['METADATA']['DATA'] = implode(",",$newdata);
	
	$meta = null;
	$metad = writexml($metadata);
	
	edit_blog_new($post_id,'save',NULL,NULL,NULL, stripslashes($metad), NULL);
}
...
function add_data_by_filename($data_type, $data_filename)
{
	...
	if(isset($ct_config['uploads_threshold']) && $ct_config['uploads_threshold'] != 0 && $size > $ct_config['uploads_threshold'])
	{
		...
		$filepath = add_data_tempnam($ct_config['uploads_dir'], 'upload_'); // fails over to use /tmp if uploads_dir is not writable, this is bad.
		move_file($data_filename, "{$ct_config['uploads_dir']}/$filepath");
...
function add_data($data_type, $data_data)
{
	...
	if(isset($ct_config['uploads_threshold']) && $ct_config['uploads_threshold'] != 0 && $size > $ct_config['uploads_threshold'] && $data_type!='data_meta')
	{
		...
		$filepath = add_data_tempnam($ct_config['uploads_dir'], 'upload_');
		file_put_contents("{$ct_config['uploads_dir']}/$filepath", $data_data);
...
function add_data_tempnam($dir, $prefix){
	
	$md5 = "";
	for($i=0;$i<100;$i++){
		$md5 = md5(microtime(true).rand(0,1000).$md5.$i);
		$name = substr($md5,10);
		$folder = substr($md5,0,2)."/".substr($md5,2,2);
		if(!file_exists("{$dir}/".$folder))
			mkdir("{$dir}/".$folder,0777,true);
		$file = "{$folder}/$prefix".substr($md5,0,10);
...
function resize_data($path,$size,$type,$idS)
{
	...
	if(!file_exists($togo))
	{
		preg_match('/\?bit=([0-9]+)$/', $path, $matches);
		if(isset($matches[1]))
		{
			$data = db_get_data_by_id($matches[1]);
			if($data['mode'] == 'filesystem')
			{
				$cmd = "convert \"".$data['data_type'].":".$data['filepath']."\" -resize $size $togo";
				`$cmd`;
			}
			else
			{
				$tmpfname = tempnam("/tmp", "blog_");
				file_put_contents ( $tmpfname, data_get_item($matches[1]) );
		...
		switch ($defaulttype) {
			...
			case 'DATA_PNG':
				...
				if(!$metadata['METADATA'][$defaulttype]['SIZE_X'] || !$metadata['METADATA'][$defaulttype]['SIZE_Y']){
					if($file_id){
						$sql_image = "SELECT mode, filepath, data_data FROM blog_data WHERE data_id = ". $file_id;
						$result_image = runQuery($sql_image,'Query image metadata');
						$row_image = db_get_next_row($result_image);						
						if($row_image['mode'] == "database")
						{
						...
						else if($row_image['mode'] == "filesystem")
						{
							list($metadata['METADATA'][$defaulttype]['SIZE_X'], $metadata['METADATA'][$defaulttype]['SIZE_Y']) = getimagesize($ct_config['uploads_dir']."/".$row_image['filepath']);
						}
...
function setposttodata($dataid,$post){
	...
	global $ct_config;		
	$sql = "SELECT data_post, data_id, data_type FROM  blog_data 	WHERE  data_id =$dataid";
	$result = runQuery($sql,'Fetch Data Item');
	if($datainfo = db_get_next_row($result)){
		if(!$datainfo['data_post']){
			$sql = "UPDATE  blog_data SET  data_post =  '$post' WHERE  blog_data.data_id = $dataid " . db_limit_1();
			runQuery($sql,'Update blog post id');
		}
		if($datainfo['data_type']=="data_meta"){
				$sql = "SELECT * FROM  blog_data 	WHERE  data_id =$dataid";
				$result = runQuery($sql,'Fetch Data Item');
				$datainfo = db_get_next_row($result);
